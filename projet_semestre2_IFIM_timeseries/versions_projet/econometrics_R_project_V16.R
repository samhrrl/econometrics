...
estimate(diff_premiere, p=5,d=0,q=3) ##le AR(4) n'est pas significatif à 5%, 
...
auto.arima(gross_ts) ##le mod�le g�n�r� automatiquement par R ne prend en compte que la partie � moyene mobile. Je pref�re avoir une composante AR
...
coeftest(fit) ##tous les r�sultats sont significatifs sauf Ar(4) et AR(5)
...
fcst <- forecast(fit, lead=4)
fcst
autoplot(fcst)

autoplot(forecast(fit, lead=4))
